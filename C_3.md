...
$$\large n = \sum_{i=0}^{d} b^i = \frac{b^{d+1} - 1}{b - 1}$$
...
###### Alpha-Beta Pruning: Making Minimax Search Efficient

Alpha-beta pruning is one of the most important optimizations for the minimax algorithm. It dramatically reduces the
number of nodes that need to be evaluated, without changing the final result.

###### The Core Concept

The key insight behind alpha-beta pruning is this: Once we know enough about a portion of the game tree to determine it
can't affect our final decision, we can skip evaluating the rest of that portion. This is similar to how you might stop
reading a restaurant menu after finding something you know you want to order.

###### How It Works

Alpha-beta pruning maintains two values as it searches:

- **Alpha**: The best (highest) value found so far for MAX along the current path
- **Beta**: The best (lowest) value found so far for MIN along the current path

When searching, if we find a move that's so good that the opponent would never let us reach it (because they have a
better alternative earlier), we can stop exploring that branch. This happens in two situations:

1. **Beta Cutoff**: When exploring a MAX node, if we find a value greater than or equal to beta, we stop exploring this
   node's remaining children. This is because the MIN player (at the parent level) would never choose this MAX node, as
   they already have a better (lower-value) option.
2. **Alpha Cutoff**: When exploring a MIN node, if we find a value less than or equal to alpha, we stop exploring this
   node's remaining children. This is because the MAX player (at the parent level) would never choose this MIN node, as
   they already have a better (higher-value) option.

###### A Simple Example

Let's look at a small example to see how pruning works:

```mermaid
graph TD
    A["A (MAX)<br>?"] --> B["B (MIN)<br>?"]
    A --> C["C (MIN)<br>?"]
...
Let's trace how alpha-beta pruning works here:

1. We start with alpha = -∞ and beta = +∞.

2. We explore node A's first child, B.

3. We explore B's first child, D.

4. We evaluate D's children: H (3) and I (5), so D gets value 5.

5. Now node B has a value of at most 5 (since the MIN player will choose the minimum).

6. We explore B's second child, E.

7. We evaluate E's first child, J (2).

8. At this point, E's value is at most 2 (since MAX will choose the maximum).

9. **Pruning happens**: Since 2 < 5, the MIN player at B would choose E over D. We don't need to explore E's other child
   K because it can't affect B's decision. We've pruned node K!

    **K=1 ?**

    Let's re-examine the scenario with node K having a value of 1 instead of 9:

    When we're at node E, we've already evaluated node J with a value of 2. Since E is a MAX node, it's trying to find
    its highest-valued child. If K were indeed 1 (which is less than J's value of 2), then E's value would still be 2
    (the maximum of [2, 1]).

    The pruning decision depends on what we already know about node B (the MIN node). We know:

    1. B has already fully evaluated child D, which has a value of 5
    2. B has partially evaluated child E, which we know has a value of at least 2 (from node J)
    3. Since B is a MIN node, it will choose the minimum of its children's values

    The key insight: Since B already has a child (E) with value 2, and 2 < 5, B would never choose D (with value 5). B's
    value will be at most 2, regardless of what other values we might find under E.

    Even if K has a value of 1, this would only lower E's value from 2 to 1, which would make E even more attractive to
    the MIN player at node B. This strengthens the decision to prune, rather than invalidating it.

    This is because pruning happens based on what we already know about a node's value, not what we might discover:

    - For a MAX node: If we find a value ≥ beta, we can prune (beta cutoff)
    - For a MIN node: If we find a value ≤ alpha, we can prune (alpha cutoff)

    In our example, when evaluating E's first child J with value 2:

    - We know E's value will be at least 2 (it could increase if K has a higher value)
    - B will take the minimum of its children's values
    - Since D has value 5 and E has value at least 2, B will prefer E (with value ≤ 2) over D (with value 5)
    - Therefore, we don't need to explore K at all - whether it's 9, 1, or any other value

    The pruning is valid because the decision at node B cannot change, regardless of K's actual value. If K is higher
    than 2, E's value increases and B still chooses E. If K is lower than 2, E's value remains 2 and B still chooses E.

    This example illustrates the power of alpha-beta pruning - it allows us to make definitive decisions about branches
    without fully exploring them, significantly reducing the computational burden without affecting the final result.

    When we're examining node E (a MAX node), we've evaluated its first child J with value 2. At this point, we know
    that E's value will be at least 2, because MAX will choose the highest value among its children.

    **K=9 ?**

    Now, regarding your question about K = 9: If we were to evaluate K and found its value is 9, then node E's value
    would become 9 (the maximum of [2, 9]). But here's the critical insight: we don't need to evaluate K at all.

    Why? Because we already know that node B (a MIN node) will choose the minimum value between its children. We've
    already fully evaluated D with value 5, and we know E has value at least 2. Since 2 < 5, B will prefer E over D no
    matter what.

    Even if E's value were to increase from 2 to 9 (after evaluating K), B would still choose E with value 2 rather than
    D with value 5. That's why we can safely prune K - its evaluation cannot change the decision at node B.

10. B gets value 2.
11. Now we explore A's second child, C.
12. We explore C's first child, F.
13. We evaluate F's first child, L (1).
14. At this point, if A were to choose C, the opponent would choose at most 1 (the value of F so far).
15. **Pruning happens again**: Since 1 < 2 (the value of B), A would never choose C over B. We can stop exploring the
    rest of node C, pruning nodes G and any other children of F!

Through pruning, we've avoided evaluating nodes K, G, and potentially many more in a larger tree.

###### The Benefits of Pruning

1. **Same Result**: Alpha-beta pruning gives exactly the same final decision as full minimax.
2. **Much Faster**: With perfect move ordering (always examining the best moves first), alpha-beta pruning allows us to
   search roughly twice as deep with the same computational effort. The number of nodes evaluated drops from O(b^d) to
   approximately O(b^(d/2)).
3. **More Efficient**: Even with imperfect move ordering, we still get substantial savings.

###### In Practice

Modern game-playing systems use alpha-beta pruning along with additional optimizations:

1. **Move Ordering**: Examining promising moves first (captures, checks, threats) increases pruning efficiency.
2. **Transposition Tables**: Remembering previously evaluated positions avoids redundant computation.
3. **Iterative Deepening**: Searching to increasing depths provides information for better move ordering.
4. **Null-Move Pruning**: Temporarily giving the opponent an extra move to quickly identify unpromising branches.

Alpha-beta pruning is what makes minimax practical for games with large branching factors. Without it, chess programs
would be limited to looking only a few moves ahead instead of the 20+ plies that modern engines can search.

In essence, alpha-beta pruning allows us to ignore parts of the game tree that simply won't matter for our final
decision, dramatically speeding up the search without sacrificing decision quality.
